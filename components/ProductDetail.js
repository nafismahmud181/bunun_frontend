  ...
  return (
    <section className="container" style={{ paddingTop: 28, paddingBottom: 64 }}>
      <div className="crumbs">
        ...
        <Link href={catHref(p.cat)}>{p.cat}</Link><span>/</span>
        <span>{p.name}</span>
      </div>
      <div className="pdp">
        <div className="gallery">
          ...
          <div className="thumbs">
            {gallery.map((g, i) => (
              <button key={g} className={`thumb ${i === img ? 'active' : ''}`} style={bg(g)} onClick={() => setImg(i)} aria-label={`Image ${i + 1}`} />
            ))}
          </div>
        </div>
        <div className="pdp-info">
          <div className="pdp-head">
            <span className="eyebrow" style={{ letterSpacing: '.06em' }}>{p.cat}</span>
            <h1>{p.name}</h1>
            <div className="pdp-meta">★ 4.8 ({40 + p.name.length * 3} reviews) · SKU BN-{p.id.toUpperCase()}-{size + 1} · <b>In stock</b></div>
          ...
          <div>
            <div className="opt-label">Size: <span>{sizes[size]}</span></div>
            <div className="sizes">
              {sizes.map((s, i) => (
                <button key={s} className={`size ${i === size ? 'active' : ''}`} onClick={() => setSize(i)}>{s}</button>
              ))}
            </div>
          </div>
          <div className="buy-row">
            ...
            <button className="btn btn-green-outline" onClick={() => { add(p.id, size, qty); setOpen(true); }}>Add to Cart</button>
            <button className="btn btn-primary" onClick={() => { add(p.id, size, qty); router.push('/checkout'); }}>Buy Now · ৳{fmt(unit * qty)}</button>
          </div>
          <div className="ship-info">
            <span><b>Inside Dhaka:</b> ৳{S.delivery.dhaka} · 1–2 days</span>
            <span><b>Outside Dhaka:</b> ৳{S.delivery.outside} · 3–5 days</span>
            <span><b>Payment:</b> COD, bKash, Nagad, Card</span>
          </div>
