...
export default function CheckoutPage() {
  ...
  const [form, setForm] = useState({ name: '', phone: '', area: 'dhaka', address: '' });
      ...
      <form className="checkout-grid" onSubmit={submit} noValidate>
        <div className="stack">
          <div className="panel">
            ...
            <input className="input" placeholder="Full name" autoComplete="name" value={form.name} onChange={set('name')} />
            <input className="input" placeholder="Mobile number (01XXXXXXXXX)" inputMode="numeric" autoComplete="tel" value={form.phone} onChange={set('phone')} />
            ...
            <input className="input" placeholder="Full address (house, road, area, district)" autoComplete="street-address" value={form.address} onChange={set('address')} />
          </div>
          <div className="panel">
            <h2>2. Payment Method</h2>
            {PAYMENTS.map(([id, label, sub]) => (
              <button type="button" key={id} className={`pay-opt ${pay === id ? 'active' : ''}`} onClick={() => setPay(id)}>
        ...
        <div className="panel summary">
          ...
          {lines.map((l) => (
            <div className="sum-line" key={l.key}>
              <span>{l.p.name} · {l.sizeLabel} × {l.qty}</span>
              <b style={{ whiteSpace: 'nowrap' }}>৳{fmt(l.total)}</b>
            </div>
          ))}
          <div className="sum-total">
            <div className="sum-line"><span>Subtotal</span><span>৳{fmt(subtotal)}</span></div>
            <div className="sum-line"><span>Delivery charge</span><span>{delivery === 0 ? 'Free' : '৳' + delivery}</span></div>
            <div className="sum-line grand"><span>Total</span><span>৳{fmt(subtotal + delivery)}</span></div>
          </div>
          <button className="btn btn-primary" type="submit" style={{ height: 50, padding: 0 }}>Place Order</button>
